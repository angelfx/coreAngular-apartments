...
export class ApartmentEditComponent implements OnInit {
  ...
  constructor(private dataService: ApartmentDataService, private router: Router, activeRoute: ActivatedRoute) {
  ...
  save() {
    this.dataService.updateProduct(this.apartment).subscribe(data => this.router.navigateByUrl("/apartments"));
  }
}
